...
class TestMatch:
    ...
    def test_match_function(self):
        ...
        import ogex
        m = ogex.match_("hello", "hello world")
        assert m is not None
        assert m.text == "hello"
...
class TestNamedGroups:
    ...
    def test_sub_named_backref(self):
        """Test sub() with named backreference \g{name}"""
        import ogex
        result = ogex.sub(r"(name:abc)", r"[\g{name}]", "abc")
        assert result == "[abc]"

    def test_sub_named_backref_multiple(self):
        """Test sub() with named backreference on multiple matches"""
        import ogex
        result = ogex.sub(r"(x:\w+)", r"<\g{x}>", "foo bar")
        assert result == "<foo> <bar>"

    def test_match_named_group(self):
        """Test named_group() method on Match object"""
        import ogex
        r = ogex.compile(r"(name:\w+)")
        m = r.search("hello")
        assert m is not None
        assert m.named_group("name") == "hello"

    def test_match_named_group_multiple(self):
        """Test named_group() with multiple named groups"""
        import ogex
        r = ogex.compile(r"(first:\w+) (last:\w+)")
        m = r.search("John Doe")
        assert m is not None
        assert m.named_group("first") == "John"
        assert m.named_group("last") == "Doe"

    def test_match_named_group_missing(self):
        """Test named_group() returns None for non-existent group"""
        import ogex
        r = ogex.compile(r"(name:\w+)")
        m = r.search("hello")
        assert m is not None
        assert m.named_group("missing") is None
